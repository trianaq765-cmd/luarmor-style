...
async function getScript() {
    ...
    try {
        const res = await axios.get(config.SCRIPT_SOURCE_URL, { timeout: 15000 }); // Reduced timeout
        if (res.data) {
...
app.get(['/loader', '/api/loader.lua', '/api/loader', '/l'], async (req, res) => {
    ...
    await logAccess(req, 'LOADER', true, { clientType: ct, userId: req.headers['x-roblox-id'] });
